...
async function clearSupabaseStorage() {
	const folders = ["clients", "users"];
	for (const folder of folders) {
		const { data: files, error } = await supabase.storage.from("invoicer-avatars").list(folder, { limit: 100 });
		if (error) throw error;
		if (!files || files.length === 0) continue;
		const paths = files.map((file) => `${folder}/${file.name}`);
		const { error: deleteError } = await supabase.storage.from("invoicer-avatars").remove(paths);
		if (deleteError) throw deleteError;
	}
	console.log("✅ Avatars bucket cleared");
}
