	...
	for (let i = 0; i < 5; i++) {
		...
		const user = await prisma.user.create({
			data: {
				...
				avatarUrl: "/uploads/avatars/users/" + faker.helpers.arrayElement(usersAvatarsFiles),
			},
		});
		...
		for (let i = 0; i < faker.number.int({ min: 5, max: 7 }); i++) {
			const client = await prisma.client.create({
				data: {
					...
					userId: user.id,
					avatarUrl: "/uploads/avatars/clients/" + faker.helpers.arrayElement(clientsAvatarsFile),
				},
			});
			clients.push(client);
		}
